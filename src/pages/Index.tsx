        ...
        {isLoading ? <p>Loading...</p> : (
          <>
            <BudgetSummary 
              ...
              savings={Number(displayAmount((budgetAmount + totalIncome) - totalExpenses))} 
            ...
            <Card>
              ...
              <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div>
                  ...
                  <p className="text-2xl font-bold">{currencySymbol}{displayAmount(cumulativeData?.totalBudget || 0)}</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground font-medium">Total Expenses</p>
                  <p className="text-2xl font-bold text-destructive">{currencySymbol}{displayAmount(cumulativeData?.totalExpenses || 0)}</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground font-medium">Total Income</p>
                  <p className="text-2xl font-bold text-success">{currencySymbol}{displayAmount(cumulativeData?.totalIncome || 0)}</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground font-medium">Total Savings</p>
                  <p className={`text-2xl font-bold ${((cumulativeData?.totalBudget || 0) + (cumulativeData?.totalIncome || 0) - (cumulativeData?.totalExpenses || 0)) >= 0 ? 'text-success' : 'text-destructive'}`}>
                    {currencySymbol}{displayAmount(((cumulativeData?.totalBudget || 0) + (cumulativeData?.totalIncome || 0) - (cumulativeData?.totalExpenses || 0)))}
                  </p>
                </div>
              </CardContent>
            </Card>
            <MotivationalTips />
            <ExpenseIncomeList transactions={transactions?.map(t => ({ ...t, amount: Number(displayAmount(Number(t.amount))) })) || []} onDelete={deleteTransaction} onEdit={(id, t, a) => updateTransaction({ id, title: t, amount: parseFloat((a / conversionRate).toFixed(6)) })} currencySymbol={currencySymbol} />
          </>
        )}
      </div>
    </div>
